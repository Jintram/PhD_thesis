...
%
%However, d
Despite earlier observations that showed a concave cAMP-growth curve with an optimum metabolic expression (supplementary figure \ref{fig:CRP:scatterspulsing} and \cite{Towbin2017}), 
we found that the shape of this cloud appears to follow a straight line with a negative slope.
%
The green cloud in the right panel in figure \ref{fig:CRP:fig2}.A shows the relationship between metabolic regulation and growth observed in cells 
that lack the ability to respond to stochastically fluctuating metabolite concentrations, ie. our $\Delta$cAMP cells growing in 800 $\upmu$M cAMP.
%
...
%
This construct carried a similar promoter to the metabolic promoter, but it contained mutations that removed the CRP binding site and introduced a $\upsigma$70 consensus binding site instead.
...
That is, the construct reports for generic protein fluctuations due to stochastic cellular state changes,
%and growth rate fluctuations, 
usually referred to as extrinsic noise --- but not due to CRP regulation.
%
Figure \ref{fig:CRP:fig2}.B shows that the expression-growth relationship of this constitutive reporter does not change in cells with or without feedback (blue and green clouds).
Thus, this confirms the attribution of the change in dynamics observed in \ref{fig:CRP:fig2}.A to the CRP regulation responding to stochastically fluctuating metabolite concentrations.

The $\Delta$cAMP cells allowed us to investigate another question:
how do the stochastic deviations \textit{from} mean behavior relate to the mean behavior itself?
%
To investigate this, we also exposed our $\Delta$cAMP cells to two additional constant conditions: minimal medium supplemented with lactose, and additionally respectively 80 or 5000 $\upmu$M cAMP.
As expected, the orange and red clouds (corresponding to lower and higher concentrations of cAMP respectively) in figure \ref{fig:CRP:fig2}.A show that the average values follow the same concave curve as observed earlier (supplementary figure \ref{fig:CRP:scatterspulsing} and \cite{Towbin2017}).
%
The clouds roughly follow this behavior, but seem to deviate slightly.
%
...
%
The average concentration seems consistent with ideas about cellular resource allocation \cite{You2013}: cells that are forced to produce more metabolic proteins, automatically produce less other proteins such as the constitutive reporter.
%
The clouds in figure \ref{fig:CRP:fig2}.B, which show the deviations of single cells from the average cell, do not follow this average behavior at all.
%
...
XXXXX-continue-here-XXXXXXXXX. 
In order to understand the shape of this curves (i.e. the clouds) we used CCs. 
...
\subsubsection*{To add tomorrow}

Despite the curve with an optimum that was observed earlier in our experiments, and also by others , 
% Surprisingly, 
cells that have a lower concentration than the average cell have a higher growth rate than average.
%
