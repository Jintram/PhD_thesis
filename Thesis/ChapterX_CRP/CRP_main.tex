...
Thus, given that the CRP system is sensitive to quickly changing input, we wanted to further probe how a population of cells growing in a constant environment responds to stochastic fluctuations .
%
Since we suspected that CRP regulation might both respond to environmental and intracellular fluctuations, 
%
we wanted to isolate and/or manipulate the regulatory response to those fluctuations.
...
%
Indeed, we managed to remove any potential response of the regulatory system to stochastic fluctuations, while maintaining the mean expression level required as response to the environment.
%
This was done by growing our $\Delta$cAMP strain on minimum medium supplemented with lactose and 800 $\upmu$M cAMP.
%
That concentration of cAMP resulted in growth rates that are comparable to those observed in wild type cells growing on lactose.
On the other hand, the cAMP levels in these cells were unable to respond to intracellular metabolite concentrations.
%
Using our previously discussed metabolic reporter construct, we were able to assess the CRP dynamics in these cells.
We compared CRP expression dynamics in these cells without feedback against wild type cells that also carried the metabolic reporter and were also growing on lactose.
%
...
%
%However, d
Despite earlier observations that showed a concave curve with an optimum enzyme expression (supplementary figure \ref{fig:CRP:scatterspulsing} and \cite{Towbin2017}), 
we found that the shape of this cloud appears to follow a straight line with a negative slope.
%
The right panel in figure \ref{fig:CRP:fig2}.A shows the relationship between metabolic regulation and growth observed in cells 
that lack the ability to respond to metabolite signals, ie. for our $\Delta$cAMP cells growing in 800 $\upmu$M cAMP.
